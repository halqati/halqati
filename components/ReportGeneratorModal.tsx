...
interface ReportGeneratorModalProps {
    ...
    onClose: () => void;
    onGenerate: (studentId: number, options: { period: string, customStart?: string, customEnd?: string, content: { [key: string]: boolean } }) => void;
...
const contentOptions = [
    ...
    { key: 'recitationDetails', label: 'تفاصيل التسميع' },
];


const ReportGeneratorModal: React.FC<ReportGeneratorModalProps> = ({ isOpen, studentId, onClose, onGenerate, setAlert }) => {
    ...
    const handleGenerate = () => {
        ...
        onGenerate(studentId, { period, customStart, customEnd, content });
    };
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <motion.div variants={modalVariants} initial="initial" animate="animate" exit="exit" className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md flex flex-col h-[90vh]">
                <div className="flex justify-between items-center mb-4 flex-shrink-0">
                    <h2 className="text-xl font-bold text-primary dark:text-accent">إنشاء تقرير طالب</h2>
                    <button onClick={onClose}><FaTimes /></button>
                </div>

                <div className="flex-grow overflow-y-auto space-y-5 pr-2">
                    <div>
                        <h3 className="font-semibold mb-2">1️⃣ اختيار الفترة الزمنية</h3>
                    ...
                    <div>
                        <h3 className="font-semibold mb-2">2️⃣ اختيار محتوى التقرير</h3>
                        <div className="grid grid-cols-2 gap-2">
                ...
                <div className="flex-shrink-0 pt-4">
                    <button onClick={handleGenerate} className="w-full px-4 py-3 rounded-lg bg-primary text-white font-bold">إنشاء وحفظ التقرير</button>
                </div>
            </motion.div>
        </div>
    );
